        ...
        if (this.exprs == null) {
            ...
            if (this.tok.getType() == Token.Type.MATH_FUNC) {
	        	...
	        	if(this.exprs.size() == 1) {
	        		DataValue notType = this.exprs.get(0).evaluate();
	        		if(notType.getType() != DataValue.Type.BOOLEAN) {
	        			throw new Exception("RUNTIME ERROR: Type mismatch - 'not' operator requires boolean operand.");
	        		}
	        		if(notType.getValue().equals(Boolean.TRUE)) {
	        			return new BooleanValue(false);
	        		} else {
	        			return new BooleanValue(true);
	        		}
	        	}
	        	else if (this.exprs.size() < 2) {
	        	...
	        	if(this.tok.toString().equals("and")) {
	        		for(int i = 0; i < this.exprs.size(); i++) {
	        			...
	        			if(this.exprs.get(i).evaluate().getValue().equals(Boolean.FALSE)) {
	        				return new BooleanValue(false);
	        			}
	        		}
	        		return new BooleanValue(true);
	        		...
	        		for(int i = 0; i < this.exprs.size(); i++) {
	        			...
	        			if(this.exprs.get(i).evaluate().getValue().equals(Boolean.TRUE)) {
	        				return new BooleanValue(true);
	        			}
	        		}
	        		return new BooleanValue(false);
